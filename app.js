...
Promise.all([
  fetch("./ncaa_report.csv?v=1",{cache:"no-store"}).then(r=>{ if(!r.ok) throw new Error("report fetch failed"); return r.text(); }),
  fetch("./projections.json?v=1",{cache:"no-store"}).then(r=>{ if(!r.ok) throw new Error("projections fetch failed"); return r.json(); })
