...
function rowsFor(player,season){
  return DATA.filter(r =>
    clean(field(r,["Player"]))===player &&
    clean(field(r,["Season"]))===season
  );
}

function metric(rows,name){
  return rows.find(r=>clean(field(r,["Metric"]))===name);
}
...
function sortRows(rows){
  return [...rows].sort((a,b)=>{
    const ca=clean(field(a,["Category"]));
    const cb=clean(field(b,["Category"]));
    const ia=CATEGORY_ORDER.indexOf(ca);
    const ib=CATEGORY_ORDER.indexOf(cb);
    const ra=ia===-1?999:ia, rb=ib===-1?999:ib;
    if(ra!==rb) return ra-rb;
    return clean(field(a,["Metric"])).localeCompare(clean(field(b,["Metric"])));
  });
}
...
function render(){
  ...
  const rows=rowsFor(player,season);
  ...
  const setStat=(id,name)=>{
    const r=metric(rows,name);
    $(id).textContent=r?fmt(field(r,["Player Value","Per-game value"]),name):"—";
  };
  setStat("toi","TOI");
  setStat("cf","CORSI For %");
  setStat("ff","Fenwick For %");
  setStat("xgf","xG For %");
  ...
  sortRows(rows).forEach(r=>{
    ...
    const m=clean(field(r,["Metric"]));
    const p=field(r,["Percentile"]);
    const pp=pctValue(p);
    ...
    tr.innerHTML=`
      <td>${category}</td>
      <td>${m}</td>
      <td>${fmt(field(r,["Player Value","Per-game value"]),m)}</td>
      <td>${fmt(field(r,["ECHL Avg"]),m)}</td>
      <td>${pp===null
        ? "N/A"
        : `<div class="pct"><span>${Math.round(pp)}%</span><span class="bar"><i style="width:${Math.max(0,Math.min(100,pp))}%"></i></span></div>`}
      </td>`;
    body.appendChild(tr);
  });

  const chartRows=CHART_METRICS
    .map(name=>metric(rows,name))
    .filter(Boolean)
    .map(r=>({
  ...
  const labelPlugin={
    ...
    afterDatasetsDraw(c){
      ...
      meta.data.forEach((bar,i)=>{
        ...
        ctx.fillText(`${Math.round(v)}%`, Math.min(bar.x+8,c.chartArea.right-34), bar.y);
      });
      ctx.restore();
    }
  };
  ...
  chart=new Chart($("profileChart"),{
    ...
    data:{
      ...
      datasets:[{data:chartRows.map(x=>x.value),borderWidth:0}]
    },
    plugins:[labelPlugin],
    options:{
      indexAxis:"y",
      ...
      plugins:{
        ...
        tooltip:{callbacks:{label:c=>`${Math.round(c.raw)}th percentile`}},
        annotation:{
          annotations:{
            medianLine:{
              type:"line",
              ...
              label:{
                ...
                position:"start",
                color:"#c7d4e4",
                backgroundColor:"rgba(11,18,32,.9)"
              }
            }
          }
        }
      ...
      scales:{
        x:{
          min:0,
          max:100,
          ticks:{callback:v=>v+"%",color:"#9fb0c6"},
          grid:{color:"#26354a"}
        },
        y:{
          ticks:{color:"#f4f7fb"},
...
function refreshPlayers(){
  ...
  const players=[...new Set(
    DATA
      .filter(r=>clean(field(r,["Season"]))===season)
      .map(r=>clean(field(r,["Player"])))
      .filter(Boolean)
  )].sort();
...
$("player").addEventListener("input",()=>{
  if([...$("players").options].some(o=>o.value===$("player").value)) render();
});

$("loadKyle").addEventListener("click",()=>{
  const seasons=[...new Set(DATA.filter(r=>clean(field(r,["Player"]))==="Kyle Neuber").map(r=>clean(field(r,["Season"]))))].filter(Boolean).sort().reverse();
  if(seasons.length){
    $("season").value=seasons.includes("2025-26")?"2025-26":seasons[0];
    refreshPlayers();
    $("player").value="Kyle Neuber";
    render();
    document.getElementById("analytics").scrollIntoView({behavior:"smooth"});
  }
});

Papa.parse("ECHL_Player_Analytics.csv",{
  download:true,
  header:true,
  dynamicTyping:false,
  skipEmptyLines:true,
  ...
  error:e=>{
    document.body.innerHTML=`<pre>Could not load CSV: ${e}</pre>`;
  }
});
