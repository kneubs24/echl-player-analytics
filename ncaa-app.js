...
Promise.all([
  fetch("./ncaa_final_year_report.csv?v=40").then(r=>{if(!r.ok)throw new Error("report");return r.text();}),
  fetch("./ncaa_final_year_comps.json?v=40").then(r=>{if(!r.ok)throw new Error("comps");return r.json();})
]).then(([text,comps])=>{
